...
- `ENDING` 使用 DOS 资源 FBP 61/62 和 MGO 571/572 播放 400 个 50ms 帧。背景每两帧
  向上拼接一行并先应用 level 2 wave，随后绘制两帧兽和四帧女孩精灵；精灵不参与 wave。
  女孩从 y=180 上升到 y=80，结束后保留最后一帧并清除 screen wave。
