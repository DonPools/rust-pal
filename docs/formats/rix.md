...
`control == 0x80` 是曲目结束：关闭全部声道，并把命令位置复位到开头。普通模式使用
九个双 operator 通道；rhythm 模式使用六个旋律通道，并按 OPL2 `0xBD` 的五个
percussion bit 驱动鼓点。
...
桌面端把 RIX 与 `MIDI.MKF` + SoundFont 作为显式可选后端；单曲缺失或损坏时返回失败，
不会静默换成另一套音色。脚本音乐编号 0 表示停止；系统菜单提供
`OFF / MIDI / RIX` 三选项。
